from mappings import sid, password_policy, lockout_policy, other_sec_options_configurations, user_rights_assignment,\
    security_options, audit_dict, reg_keys
...
def privilege_rights_to_dict():
    try:
        ...
        for conf, value in raw_local_p_user_right_dict.items():
            latest = {user_rights_assignment.get(conf): value}
    ...
    try:
        ...
        with open(f'{sec_edit_file}', 'r', encoding='utf16') as sec_options:
            for line in sec_options.readlines()[start:end]:
                ...
                configs = line[0]
                status = line[1].split(',')
                for k in security_options.keys():
                    if configs.__contains__(k):
                        index = configs.find(k)
                        if status[0] in reg_keys.keys():
                            status = [reg_keys.get(status[0]), reg_keys.get(status[1])]
                            mapped = {security_options.get(configs[index:]): status}
    ...
    try:
        ...
        with open(f'{sec_edit_file}', 'r', encoding='utf16') as sys_access:
            for line in sys_access.readlines()[start:end]:
                line = line.strip('\n').replace(' = ', ':').split(':')
                if line[0] in other_sec_options_configurations.keys():
                    keys = other_sec_options_configurations.get(line[0])
                    vals = line[1]
                    new_dict = {keys: vals}
...
def password_policy_to_dict():
    try:
        ...
        with open(f'{sec_edit_file}', 'r', encoding='utf16') as sys_access:
            for line in sys_access.readlines()[start:end]:
                line = line.strip('\n').replace(' = ', ':').split(':')
                if line[0] in password_policy.keys():
                    keys = password_policy.get(line[0])
                    vals = line[1]
...
def lockout_policy_to_dict():
    try:
        ...
        with open(f'{sec_edit_file}', 'r', encoding='utf16') as sys_access:
            for line in sys_access.readlines()[start:end]:
                line = line.strip('\n').replace(' = ', ':').split(':')
                if line[0] in lockout_policy.keys():
                    keys = lockout_policy.get(line[0])
                    vals = line[1]
                    new_dict = {keys: int(vals)}
    ...
    try:
        ...
        with open(f'{sec_edit_file}', 'r', encoding='utf16') as audit:
            for line in audit.readlines()[start:end]:
                line = line.strip('\n').replace(' = ', ':').split(':')
                if line[0] in audit_dict.keys():
                    keys = audit_dict.get(line[0])
                    vals = line[1]
                    new_dict = {keys: int(vals)}
                    raw_local_p_user_right_dict.update(new_dict)

            print(colored(raw_local_p_user_right_dict, 'red'))
